...
- Conversation detail opens on the latest 40-message page, including cache hydration. Older history loads only when the user scrolls to the top, and prepending a page preserves the current reading position.
...
- Public QR generation prefers the configured `publicUrl` (`--public-url` or `CODEX_LAN_PUBLIC_URL`) over detected LAN addresses, keeping reverse-proxy deployments on their public HTTPS origin.
- Login QR access codes are carried in the URL fragment rather than the query string; the web client still accepts legacy `login`/`token` query parameters for compatibility.
- Additional computer: `homepc` runs on Tailscale host `736-win` at `100.109.17.103:8787`; its direct mobile entry requires the phone to be connected to the same Tailnet.
- Additional computer public entry (verified September 4, 2026): `homepc` is reachable through Alibaba Cloud HTTPS port `18789`; reverse path is public Nginx `18789` -> loopback `18788` -> dedicated reverse SSH tunnel -> Windows `8787`. The Alibaba security-group rule for TCP `18789` is enabled. The phone must use the generated `codexpocket://add` QR for this public URL, not the old Tailscale QR.
...
## 2026-09-07 reliability fixes

- Mobile sends now retain a background Desktop refresh job when the Desktop IPC is unavailable; jobs retry after recovery without adding stale notices to conversations.
...
- Insert errors that report an already-ended Desktop turn are treated as a race and fall back to a normal turn. Queued messages expose authenticated image previews; editing a queued item returns its original text and image data and removes the stale optimistic bubble.
- Queue follow-ups remain visible only in the composer queue panel; queued optimistic chat bubbles are filtered out. Each queued item can be atomically converted to an insert through `/api/queue/steer`, restoring the item if insertion fails. Rollout parsing accepts image arrays and alternate image content fields so multiple uploaded images remain attached to one user message.
